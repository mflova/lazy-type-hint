...
class DictMetadata:
    ...
    """Preffix prepended to a key to indicate this one should be hidden when building its type alias."""
    ...
    def get_keys(self, *, include_hidden_prefix_keys: bool = False) -> Set[Hashable]:
        ...
        if self._all_keys_are_string(self._data):
            return {key for key in self._data if not key.startswith(self.hidden_key_preffix)}
        return set(self._data.keys())
...
{TAB}{{key}}: {{value}}{{optional_key_docstring}}"""
    ...
    def from_multiple_dict_data_type_trees(cls, *trees: "DictDataTypeTree") -> "Self":
        ...
        For thsi to happen, two main tasks are carried out:
