...
class TestInstantiation:
    @pytest.mark.parametrize(
        ...
        [
            ...
            (pd.DataFrame({"A": [1]}), (pd.DataFrame({("A",): [1, 2, 3]}))),
        ],
    )
...
class TestGetStrPy:
    ...
    STRATEGY: Final = ParsingStrategies(pandas_type_hint_columns=True)
...
class TestHash:
    ...
    @pytest.mark.parametrize(
        ...
        [
            (pd.DataFrame(), pd.DataFrame(), ParsingStrategies(pandas_type_hint_columns=True) ,True),
            (pd.DataFrame({"A": [1,2,3]}), pd.DataFrame(), ParsingStrategies(pandas_type_hint_columns=True) ,False),
            (pd.DataFrame({"A": [1,2,3]}), pd.DataFrame({"A": [1,2,3]}), ParsingStrategies(pandas_type_hint_columns=True) ,True),
            (pd.DataFrame({"A": [1,2,3]}), pd.DataFrame({"B": [1,2,3]}), ParsingStrategies(pandas_type_hint_columns=True) ,False),
            (pd.DataFrame({("A", "B"): [1,2,3]}), pd.DataFrame({"B": [1,2,3]}), ParsingStrategies(pandas_type_hint_columns=True) ,False),
            (pd.DataFrame(), pd.DataFrame(), ParsingStrategies(pandas_type_hint_columns=False) ,True),
            (pd.DataFrame({"A": [1,2,3]}), pd.DataFrame(), ParsingStrategies(pandas_type_hint_columns=False) , True),
            (pd.DataFrame({"A": [1,2,3]}), pd.DataFrame({"A": [1,2,3]}), ParsingStrategies(pandas_type_hint_columns=False) ,True),
            (pd.DataFrame({"A": [1,2,3]}), pd.DataFrame({"B": [1,2,3]}), ParsingStrategies(pandas_type_hint_columns=False) , True),
            (pd.DataFrame({("A", "B"): [1,2,3]}), pd.DataFrame({"B": [1,2,3]}), ParsingStrategies(pandas_type_hint_columns=False) , True),
        ],
    )
    # fmt: on
    def test_get_str_top_node(
