...
from lazy_type_hint.lazy_type_hint import LazyTypeHint, LazyTypeHintError
...
class TestLazyTypeHint:
    DATA_TEST_DIR: Final = Path(__file__).parent / "test_files"

    @pytest.fixture
    def lazy_type_hint(self) -> LazyTypeHint:
        return LazyTypeHint()
    ...
    def test_exceptions_raised(self, file: str) -> None:
        file_path = self.DATA_TEST_DIR / file
        with pytest.raises(LazyTypeHintError):
