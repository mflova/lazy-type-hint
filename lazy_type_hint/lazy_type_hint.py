...
class LazyTypeHint:
    ...
    def _validate_classes_custom_dir(
        ...
        if not all(isinstance(element, str) for element in generated_classes_custom_dir[1:]):
            raise LazyTypeHintError(f"All elements of `{arg_name}` (but the first one) must be strings.")
    ...
    def _add_overload_to_this_file_pyi(
        ...
        if not self.this_file_pyi.search_decorator(decorator_name="overload", method_name=method_name):
            ...
            if not idx_lst:
                raise LazyTypeHintError(f"No method `{method_name}` could be found")
        ...
        if first_idx == -1:
            raise LazyTypeHintError(f"No {input_argument} could be found within the signature {signature}")
    ...
    def _add_class_created_to_this_file_pyi(self, new_class: str) -> None:
        ...
        if not value:
            raise LazyTypeHintError(f"No `{label}` was found in this file.")
        # Case: Any
        if value == "Any":
